  ...
  build:
    ...
    steps:
      ...
      - name: Apply semantic version
        ...
        uses: spritely/actions.semantic-version/apply@v0.5.6
        with:
...
jobs:
  build:
    ...
    steps:
      - name: Build and publish container
        uses: spritely/build/container@v0.22.1
        with:
...
jobs:
  build:
    ...
    steps:
      - name: Build and publish .NET container
        uses: spritely/build/dotnet-container@v0.22.1
        with:
          registryUsername: ${{ github.actor }}
          registryPassword: ${{ github.token }}
          # Always prefix all names with the registry host name (an optional parameter that defaults to ghcr.io)
  ...
  build:
    ...
    steps:
      - name: Build and publish dotnet package
        uses: spritely/build/dotnet-package@v0.22.1
        with:
          registryUsername: ${{ github.actor }}
          registryPassword: ${{ github.token }}
  ...
  build:
    ...
    steps:
      - name: Build and test dotnet project
        uses: spritely/build/dotnet-test@v0.22.1
        with:
          registryUsername: ${{ github.actor }}
          registryPassword: ${{ github.token }}
          projectFile: MyProject.csproj
